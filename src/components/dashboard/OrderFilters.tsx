...
export async function OrderFilters({ initial }: OrderFiltersProps) {
  const t = await getTranslations('dashboard.orderFilters')

  return (
    <form className="grid grid-cols-1 gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-6">
      <div className="md:col-span-2">
        <label htmlFor="search" className="text-xs font-medium text-gray-600">{t('searchLabel')}</label>
        <input
          id="search"
          name="search"
      ...
      <div>
        <label htmlFor="paymentMethod" className="text-xs font-medium text-gray-600">{t('paymentLabel')}</label>
      ...
      <div>
        <label htmlFor="dateFrom" className="text-xs font-medium text-gray-600">{t('fromLabel')}</label>
      ...
      <div>
        <label htmlFor="dateTo" className="text-xs font-medium text-gray-600">{t('toLabel')}</label>
      ...
      <div className="md:col-span-6">
        <button type="submit" className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white">{t('applyButton')}</button>
      </div>
    </form>
  )
}
